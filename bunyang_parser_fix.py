...
def extract_dong_ho(text: str):
    ...
    OCR이 표지/본문/부속계약서에서 같은 동호를 여러 번 인식하는 경우
    첫번째 매치가 오인식일 수 있으므로 모든 매치의 다수결로 최종값을 결정한다.
    """
    matches = re.findall(r'(\d{3,4})\s*동\s*(\d{3,4})\s*호', text)
    if matches:
        winner = Counter(matches).most_common(1)[0][0]
...
def extract_type(text: str) -> str:
    ...
    OCR이 '108A'를 '10840'으로 깨뜨리는 경우 '108.1488A' 형식(면적+타입)에서 폴백 추출.
    예: '108.1488A"  검단신도시...' → '108A'
    """
    # 1순위: 타입 코드 + Type 키워드
    m = re.search(r'(\d{2,3}[A-Za-z]?)\s*[Tt]ype', text)
    if m:
        return m.group(1).upper()
    # 2순위: 발코니/선택품목계약서 헤더 '108.1488A' 형식
    m2 = re.search(r'(\d{2,3})\.\d{3,4}([A-Za-z])', text)
    if m2:
        return m2.group(1) + m2.group(2).upper()
    # 3순위: '84A㎡' 형식
    m3 = re.search(r'(\d{2,3}[A-Za-z])\s*㎡', text)
    if m3:
        return m3.group(1)
    return ""
...
def extract_contractors(text: str) -> list:
    ...
    [케이스 3] 개명 처리
    '변경전 성명: 김아름' → 현재: 김태은 (이전 이름 무시)
    """
    # 개명 케이스: 주민등록초본의 '변경전 성명' 이전 이름을 먼저 제거
    clean_text = re.sub(r'변경\s*전\s*성\s*명\s*[：:]\s*[가-힣]{2,5}', '', text)
    clean_text = re.sub(r'전\s*성\s*명\s*[：:]\s*[가-힣]{2,5}', '', clean_text)
...
def korean_amount_to_int(text: str) -> int:
    ...
    억/만 단위 경계에서 분리 후 각 구간을 독립 계산.
    구버전 로직은 '팔천삼백' 처리 시 천 이후 자리를 10배로 곱하는 버그가 있었음.
    """
